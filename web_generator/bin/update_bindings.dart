...
Future<Map<String, List<String>>> loadGenforConfig(String path) async {
  ...
  // Cast the map properly
  return (jsonMap as Map<String, dynamic>).map(
...
void main(List<String> arguments) async {
    ...
    try {
        ...
        print('''${ansi.lightRed.wrap(e.message)} $_usage''');
    ...
    final generateArgs = argResult.command!;
    final isAll = generateArgs['all'] as bool? ?? false;
    final pathArg = generateArgs['path'] as String?;

    if (argResult.command?.name != 'generate') {
        print('Only the "generate" command is supported.');
        print('\nUsage:\n');
        print(_parser.usage);
        print('\nGenerate command:\n');
        print(_parser.commands['generate']!.usage);
        return;
    }
    ...
    if (isAll) {
        ...
        existingFiles =
            domDir.listSync(recursive: true).whereType<File>().where((file) {
                    ...
                    return 
                        contents.contains('Generated from Web IDL definitions');
                    }).toList();
    } else if (pathArg != null) {
        final jsonFile = File(pathArg);
        if (!jsonFile.existsSync()) {
            print('Error: JSON file not found at path: $pathArg');
            return;
        }

        try {
            final jsonContent = jsonFile.readAsStringSync();
            final List<dynamic> filePaths = json.decode(jsonContent);
            if (filePaths.isEmpty) {
                print('Error: JSON file is empty or contains no file paths');
                return;
            }

            // Convert the paths to File objects
            existingFiles = filePaths.map((path) => File(path as String)).toList();
        ...
        print('Missing required flag: either --all '
    ...
    final timeStamps =  {
        for (final file in existingFiles) 
            if (file.existsSync()) file.path: file.lastModifiedSync(),
    };
    ...
    await _runProc(
            ...
            if (isAll) '--files=[]',
            if (pathArg != null) '--files=${json.encode(existingFiles.map((file) => p.relative(file.path, from: p.join(_webPackagePath, 'lib', 'src'))).toList())}',
            ],
            workingDirectory: _bindingsGeneratorPath,
            );
...
final _generateCommand = ArgParser()
    ..addFlag('all',
        abbr: 'a',
        help: 'Generate bindings for all IDL definitions',
        negatable: false)
    ..addOption('path',
        abbr: 'p',
        help: 'Path to a JSON file containing an array of file paths to generate',
        valueHelp: 'path/to/files.json');

final _parser = ArgParser()
    ..addCommand('generate', _generateCommand)
